<div align="center">
  <!-- HOSTED HEADER (Guaranteed to load) -->
  <img src="https://raw.githubusercontent.com/rajat0809/rajat0809/main/header.svg" width="100%" alt="Header" />
</div>

<div align="center">
  <h3>
    <a href="mailto:rajatchandak0809@gmail.com">
      <img src="https://img.shields.io/badge/Available_For-Senior_Roles_%7C_Consulting-success?style=for-the-badge&logo=clock&logoColor=white" />
    </a>
    <a href="https://www.linkedin.com/in/chandakrajat0809">
      <img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
    </a>
    <a href="https://rajat0809.github.io/">
      <img src="https://img.shields.io/badge/Experience-5%2B_Years-blue?style=for-the-badge&logo=github&logoColor=white" />
    </a>
  </h3>
</div>

---

## 👨‍💻 **About Me**

**Senior Full Stack Developer & Consultant** with 5+ years of experience architecting scalable fintech solutions. I specialize in building high-performance systems using **Java, Go, Angular, and Cloud Architecture**.

> **"Bridging the gap between complex business requirements and robust technical solutions."**

- 🔭 **Roles**: Senior Tech Lead, Solution Architect, Expert Consultant
- 💼 **Availability**: Open for **Senior SDE/Lead roles** (Full-time) or **Expert Consulting** (Contractual)
- 🎓 **Education**: B.Tech, The LNM Institute of Information Technology (2016-2020)
- 📍 **Location**: Delhi, India

---

## 🛠️ **Technical Leadership Stack**

| **Core Backend** | **Modern Frontend** | **Architecture & DevOps** |
|:---|:---|:---|
| ![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white) **Java / Spring Boot** | ![Angular](https://img.shields.io/badge/Angular-DD0031?style=flat&logo=angular&logoColor=white) **Angular 18** | ![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white) **Docker / K8s** |
| ![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white) **Go (Golang)** | ![TS](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white) **TypeScript** | ![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white) **Cloud Architecture** |
| ![Node](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white) **Node.js** | ![RxJS](https://img.shields.io/badge/RxJS-B7178C?style=flat&logo=reactivex&logoColor=white) **RxJS** | ![Kafka](https://img.shields.io/badge/Kafka-231F20?style=flat&logo=apachekafka&logoColor=white) **Event-Driven** |
| ![Postgres](https://img.shields.io/badge/Postgres-4169E1?style=flat&logo=postgresql&logoColor=white) **PostgreSQL** | ![Material](https://img.shields.io/badge/Material-0081CB?style=flat&logo=materialdesign&logoColor=white) **Material Design** | ![CI/CD](https://img.shields.io/badge/CI%2FCD-Jenkins-D24939?style=flat&logo=jenkins&logoColor=white) **Pipelines** |

## 💼 **Services & Engagement**

I help companies build **bank-grade fintech infrastructure**.

🔹 **Senior Tech Lead**: Leading teams to deliver complex payment integrations.  
🔹 **Architectural Consulting**: Designing microservices, event-driven systems (Kafka), and legacy migrations.  
🔹 **Freelance Delivery**: End-to-end development of secure API gateways and dashboards.  

---

<div align="center">
  <h3>
    <a href="mailto:rajatchandak0809@gmail.com">📫 Contact for Consulting or Roles</a>
  </h3>
</div>
